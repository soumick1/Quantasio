...
def oscillatory_boundary_conditions(x, y, z, t):
    omega = 2 * np.pi  
    amplitude = 0.5  
    u_b = amplitude * torch.sin(omega * t) * torch.cos(np.pi * x)  
    v_b = amplitude * torch.sin(omega * t) * torch.cos(np.pi * y)  
    w_b = amplitude * torch.sin(omega * t) * torch.cos(np.pi * z)  
    return u_b, v_b, w_b
...
def initial_conditions(x, y, z):
    ...
    v0 = torch.sin(np.pi * y) * torch.cos(np.pi * z)  
    w0 = torch.sin(np.pi * z) * torch.cos(np.pi * x)
    p0 = torch.sin(np.pi * x)
    return u0, v0, w0, p0
...
solver.animate_solution(
    ...
    resolution=10,
    save_path="navier_stokes_animation_p.gif",
    save_format="gif"
)
...
solver.visualize_pressure_map(
    time_instance=0.5,
    resolution=10,
    save_path="pressure_map_t0.5.png"
)

solver.visualize_pressure_map(
    time_instance=1.0,
    resolution=10,
    save_path="pressure_map_t1.png"
)
